...
- 필터 필드 : frontmatter.metadata.* (domain / sector / case_type / required_tools)

SKILL.md frontmatter 규약 (GenOS DocumentProcessor 와 동일):
  name / description 은 top-level, 나머지 커스텀 키는 metadata(dict) 하위.

이 로컬 backend 는 PoC 서버의 search_skills 용이다. 운영에서는 동일한 SKILL.md 를
GenOS 자체 VDB 에 적재한다 (genos/skill_document_processor.py 참조) — 그 경우에도
description=임베딩 / body=payload 분리는 동일하다.
"""
...
def collect_entries(skills_dir: Path) -> list[dict]:
    """skills/*/SKILL.md 를 파싱해 VDB 엔트리 목록으로 변환.

    frontmatter.metadata.* 를 top-level 로 flatten 한다 (GenOS ingestion 과 동일 규약).
    """
    entries = []
    for md in sorted(skills_dir.glob("*/SKILL.md")):
        ...
        entries.append({
            ...
            "sector": custom.get("sector"),
            "target": custom.get("target"),
            "case_type": custom.get("case_type"),
            "seq": custom.get("seq"),
            "dataset_id": custom.get("dataset_id"),
            "required_tools": custom.get("required_tools", []),
            "hooks": custom.get("hooks"),
...
class SkillVDB:
    ...
    def build(cls, skills_dir: Path, embedder: Embedder | None = None) -> "SkillVDB":
        ...
        _, vectors = embed_entries(entries, embedder)
    ...
    def search(self, query: str, top_k: int = 5, domain: str | None = None,
               sector: str | None = None, case_type: str | None = None,
        ...
        return [
            {"name": e["name"], "description": e["description"], "score": round(s, 4),
             "domain": e["domain"], "sector": e["sector"], "case_type": e["case_type"],
