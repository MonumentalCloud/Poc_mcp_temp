...
보험금 청구 요청을 받아 청구 가능한 관계사 화면으로 연결한다.

## Instructions
1. 사용자 질의에서 청구 대상 계약(회사/상품)을 파악합니다.
...
4. 사용자가 동의하면 `insurance_claim_submit` 툴을 호출해 보험금 청구 프로세스를 시작합니다 (실행형, 관계사 화면 이동).
5. 모니모에서 직접 청구는 지원되지 않으므로, 청구 가능한 회사(생명/화재) 화면으로 이동 배너를 제공합니다.
...
- `insurance_contract_inquiry(company: Optional[str] = None)` — 사용자가 보유한 보험 계약 목록을 조회합니다
- `insurance_claim_submit(company: str, claim_type: Optional[str] = None)` — 보험금 청구 프로세스를 시작합니다 (실행형, 관계사 화면 이동)

## 응답 가이드
- 보험금 청구 가능한 회사의 화면으로 이동(생명or화재포함)- 모니모에서 지원X
...
```python
# -*- coding: utf-8 -*-
"""Hook script for skill `insurance_claim_submit` (자동 생성).
...
SKILL_NAME = 'insurance_claim_submit'
CASE_TYPE = 'normal'
FLOW = 'action'
REQUIRED_TOOLS = ['insurance_contract_inquiry', 'insurance_claim_submit']
ACTION_TOOLS = ['insurance_claim_submit']    # 사용자 확인(confirmed=True) 없이는 호출 금지
PHRASES = ['안내: "보험금 청구는 {회사} 화면에서 진행할 수 있어요. 아래 배너로 이동해 청구를 계속해 주세요."', '대상 없음: "청구 가능한 보험 계약을 찾지 못했어요. 어떤 보험의 보험금을 청구하시려는지 알려주시겠어요?"']
