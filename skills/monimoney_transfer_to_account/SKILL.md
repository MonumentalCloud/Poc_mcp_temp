...
- `monimoney_withdraw(amount: int, target_account: Optional[str] = None)` — 모니머니를 지정 계좌로 출금(이체)합니다 (실행형)
...
```python
# -*- coding: utf-8 -*-
"""Hook script for skill `monimoney_transfer_to_account` (자동 생성).
...
SKILL_NAME = 'monimoney_transfer_to_account'
CASE_TYPE = 'normal'
FLOW = 'action'
REQUIRED_TOOLS = ['monimoney_balance_inquiry', 'monimoney_withdraw']
ACTION_TOOLS = ['monimoney_withdraw']    # 사용자 확인(confirmed=True) 없이는 호출 금지
