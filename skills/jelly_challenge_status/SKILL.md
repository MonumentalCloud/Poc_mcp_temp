...
- `challenge_status_inquiry(challenge_name: Optional[str] = None)` — 참여 중인 특정 챌린지의 달성 현황(달성일수 등)을 조회합니다
...
```python
# -*- coding: utf-8 -*-
"""Hook script for skill `jelly_challenge_status` (자동 생성).
...
SKILL_NAME = 'jelly_challenge_status'
CASE_TYPE = 'normal'
FLOW = 'query'
REQUIRED_TOOLS = ['challenge_status_inquiry']
