...
3. `monischool_hint_inquiry` 툴을 호출해 특정 교시 문제의 힌트를 조회합니다.
4. 조회 결과를 달성/미달성 구분 등 세부 기준과 함께 안내하고, 해당 서비스 화면 이동 배너를 제공합니다.
...
- `monischool_hint_inquiry(session: int)` — 특정 교시 문제의 힌트를 조회합니다

## 응답 가이드
- 해당 문제의 힌트 내용 제공 및 힌트 화면 랜딩 제공
...
```python
# -*- coding: utf-8 -*-
"""Hook script for skill `monischool_quiz_hint` (자동 생성).
...
SKILL_NAME = 'monischool_quiz_hint'
CASE_TYPE = 'normal'
FLOW = 'query'
REQUIRED_TOOLS = ['monischool_quiz_inquiry', 'monischool_hint_inquiry']
