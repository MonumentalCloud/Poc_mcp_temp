...
- `event_participation_inquiry(event_id: str)` — 특정 이벤트에 대한 사용자의 참여/응모 여부를 조회합니다
...
```python
# -*- coding: utf-8 -*-
"""Hook script for skill `event_participation_check` (자동 생성).
...
SKILL_NAME = 'event_participation_check'
CASE_TYPE = 'normal'
FLOW = 'query'
REQUIRED_TOOLS = ['event_list_inquiry', 'event_participation_inquiry']
ACTION_TOOLS = []    # 사용자 확인(confirmed=True) 없이는 호출 금지
PHRASES = ['참여함: "{이벤트명}에 {일자}에 응모하셨어요."', '참여 안 함: "{이벤트명} 응모 내역이 없어요. 지금 참여해 보시겠어요?"']
