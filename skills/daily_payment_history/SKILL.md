...
- `payment_history_inquiry(target_date: Optional[str] = None, merchant: Optional[str] = None)` — 특정 일자/기간의 결제이력(명세서 단위)을 조회합니다
...
```python
# -*- coding: utf-8 -*-
"""Hook script for skill `daily_payment_history` (자동 생성).
...
SKILL_NAME = 'daily_payment_history'
CASE_TYPE = 'normal'
FLOW = 'query'
REQUIRED_TOOLS = ['payment_history_inquiry']
