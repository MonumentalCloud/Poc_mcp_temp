...
2. `spending_by_card_inquiry` 툴을 호출해 기준월의 카드별 소비금액 합계를 조회합니다.
...
- `spending_by_card_inquiry(year_month: Optional[str] = None, card_name: Optional[str] = None)` — 기준월의 카드별 소비금액 합계를 조회합니다

## 응답 가이드
- 지난달 소비금액 총합이 가장 낮은 카드
...
```python
# -*- coding: utf-8 -*-
"""Hook script for skill `monthly_card_usage_ranking` (자동 생성).
...
SKILL_NAME = 'monthly_card_usage_ranking'
CASE_TYPE = 'normal'
FLOW = 'query'
REQUIRED_TOOLS = ['spending_by_card_inquiry']
