...
- `contract_withdrawal_request(contract_id: str)` — 신계약 청약철회를 진행합니다 (실행형)
...
```python
# -*- coding: utf-8 -*-
"""Hook script for skill `contract_withdrawal_request` (자동 생성).
...
SKILL_NAME = 'contract_withdrawal_request'
CASE_TYPE = 'normal'
FLOW = 'action'
REQUIRED_TOOLS = ['insurance_contract_inquiry', 'contract_withdrawal_request']
ACTION_TOOLS = ['contract_withdrawal_request']    # 사용자 확인(confirmed=True) 없이는 호출 금지
