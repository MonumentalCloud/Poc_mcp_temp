...
2. `insurance_premium_estimate` 툴을 호출해 보험상품의 연령대·성별 기준 예상 보험료를 조회합니다.
3. 예상 보험료를 산출 기준(연령/성별/만기)과 함께 안내합니다. 예: 30대 여성 월 보험료 45,440원(10년 만기 기준).
4. 연령·성별·보장범위에 따라 달라지는 단순 예상치임을 반드시 밝히고, 정확한 보험료는 상품별 조회가 필요함을 안내합니다.
...
## 응답 가이드
- 연령/성별/보장범위에 따라 달라진다는 점 안내
- 정확 보험료는 상품별 조회 필요 안내
...
```python
# -*- coding: utf-8 -*-
"""Hook script for skill `calc_insurance_premium` (자동 생성).
...
SKILL_NAME = 'calc_insurance_premium'
CASE_TYPE = 'normal'
FLOW = 'query'
REQUIRED_TOOLS = ['insurance_premium_estimate']
