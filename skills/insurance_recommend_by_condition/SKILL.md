...
- `product_recommend(product_type: str, preference: Optional[str] = None)` — 사용자 조건·선호(혜택, 목적 등)에 맞는 상품 후보를 조회합니다
...
```python
# -*- coding: utf-8 -*-
"""Hook script for skill `insurance_recommend_by_condition` (자동 생성).
...
SKILL_NAME = 'insurance_recommend_by_condition'
CASE_TYPE = 'normal'
FLOW = 'query'
REQUIRED_TOOLS = ['product_recommend', 'product_detail']
ACTION_TOOLS = []    # 사용자 확인(confirmed=True) 없이는 호출 금지
PHRASES = ['추가 질문: "어떤 용도로 찾고 계세요? {조건 선택지}"', '안내: "말씀하신 조건에는 {후보 상품들}이 있어요. 각각 {특징 요약}입니다."']
