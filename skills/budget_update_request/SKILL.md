...
2. `budget_inquiry` 툴을 호출해 사용자가 설정한 한 달 예산과 현재 소비 대비 현황을 조회합니다.
3. 실행 내용(대상/금액/기간)을 사용자에게 요약해 보여주고 진행 여부를 확인받습니다. 확인 없이 실행하지 않습니다.
4. 사용자가 동의하면 `budget_update` 툴을 호출해 사용자의 한 달 예산 금액을 변경합니다.
...
- `budget_update(new_amount: int)` — 사용자의 한 달 예산 금액을 변경합니다 (실행형)

## 응답 가이드
- 예산 변경 화면으로 이동가능한 배너 제공
...
```python
# -*- coding: utf-8 -*-
"""Hook script for skill `budget_update_request` (자동 생성).
...
SKILL_NAME = 'budget_update_request'
CASE_TYPE = 'normal'
FLOW = 'action'
REQUIRED_TOOLS = ['budget_inquiry', 'budget_update']
ACTION_TOOLS = ['budget_update']    # 사용자 확인(confirmed=True) 없이는 호출 금지
