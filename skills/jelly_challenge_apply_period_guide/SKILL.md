---
name: jelly_challenge_apply_period_guide
...
2. `challenge_list_inquiry` 툴을 호출해 참여 중이거나 신청 가능한 젤리 챌린지 목록과 신청 가능 기간을 조회합니다.
3. 조회 결과를 일반젤리/스페셜젤리 구분 등 세부 기준과 함께 안내하고, 젤리 상점·챌린지 화면 이동 배너를 제공합니다.
...
## 응답 가이드
- 챌린지 신청 가능 기간 안내 (매월 16일~말일 이내에만 차월 챌린지 신청 가능)- 젤리 챌린지 메인 화면으로 이동
...
```python
# -*- coding: utf-8 -*-
"""Hook script for skill `jelly_challenge_apply_period_guide` (자동 생성).
...
SKILL_NAME = 'jelly_challenge_apply_period_guide'
CASE_TYPE = 'normal'
FLOW = 'query'
