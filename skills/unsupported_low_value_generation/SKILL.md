---
name: unsupported_low_value_generation
description: 사용자가 '아무 질문이나 해줘' 등 의미 없는 텍스트 생성을 요청한 경우 사용합니다.
metadata:
  domain: unsupported
  sector: non_finance
  case_type: error
...
```python
# -*- coding: utf-8 -*-
"""Hook script for skill `unsupported_low_value_generation` (자동 생성).
...
SKILL_NAME = 'unsupported_low_value_generation'
CASE_TYPE = 'error'
FLOW = 'guardrail'
REQUIRED_TOOLS = []
ACTION_TOOLS = []    # 사용자 확인(confirmed=True) 없이는 호출 금지
PHRASES = ['제한 안내: "죄송하지만 요청하신 내용은 도와드릴 수 없어요. 모니모 서비스와 금융 관련 질문을 도와드릴게요."']
