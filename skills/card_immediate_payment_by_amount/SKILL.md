...
2. `card_billing_inquiry` 툴을 호출해 이번 결제일 기준 결제예정금액을 조회합니다.
...
- `card_immediate_payment(amount: Optional[int] = None, tx_ids: Optional[list[str]] = None)` — 카드 이용대금을 금액 기준 또는 건별로 즉시결제합니다 (실행형, 삼성카드만)
...
```python
# -*- coding: utf-8 -*-
"""Hook script for skill `card_immediate_payment_by_amount` (자동 생성).
...
SKILL_NAME = 'card_immediate_payment_by_amount'
CASE_TYPE = 'normal'
FLOW = 'action'
REQUIRED_TOOLS = ['card_billing_inquiry', 'card_immediate_payment']
