...
2. `kelly_history_inquiry` 툴을 호출해 기준월의 켈리 적립/사용 내역을 조회합니다.
...
- `kelly_history_inquiry(year_month: Optional[str] = None)` — 기준월의 켈리 적립/사용 내역을 조회합니다

## 응답 가이드
- 현재 일 기준, '켈리보관함' 내 켈리 개수 합산하여 안내 ex) 'YY.MM...
...
```python
# -*- coding: utf-8 -*-
"""Hook script for skill `kelly_earning_history` (자동 생성).
...
SKILL_NAME = 'kelly_earning_history'
CASE_TYPE = 'normal'
FLOW = 'query'
REQUIRED_TOOLS = ['kelly_history_inquiry']
