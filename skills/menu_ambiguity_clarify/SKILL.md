...
```python
# -*- coding: utf-8 -*-
"""Hook script for skill `menu_ambiguity_clarify` (자동 생성).
...
SKILL_NAME = 'menu_ambiguity_clarify'
CASE_TYPE = 'normal'
FLOW = 'query'
REQUIRED_TOOLS = ['menu_search']
ACTION_TOOLS = []    # 사용자 확인(confirmed=True) 없이는 호출 금지
PHRASES = ['의도 확인: "어떤 포인트를 말씀하시는 걸까요? (갤러/모니머니/카드포인트)"', '안내 성공: "{항목}은(는) {경로}에서 사용하실 수 있어요."']
