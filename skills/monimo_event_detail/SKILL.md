...
- `event_list_inquiry(company: Optional[str] = None, keyword: Optional[str] = None)` — 진행 중인 이벤트 목록을 회사·키워드 필터로 조회합니다
- `event_detail_inquiry(event_id: str)` — 특정 이벤트의 세부 내용(기간/혜택/발표일/응모방법)을 조회합니다

## 응답 가이드
- 특정이벤트 요약정보 안내 및 발표일 안내
...
```python
# -*- coding: utf-8 -*-
"""Hook script for skill `monimo_event_detail` (자동 생성).
...
SKILL_NAME = 'monimo_event_detail'
CASE_TYPE = 'normal'
FLOW = 'query'
REQUIRED_TOOLS = ['event_list_inquiry', 'event_detail_inquiry']
