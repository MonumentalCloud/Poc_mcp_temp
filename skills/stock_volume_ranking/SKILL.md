...
- `stock_ranking_inquiry(criteria: str = 'popular', sector: Optional[str] = None)` — 인기/거래량/거래대금/업종별 종목 순위를 조회합니다

## 응답 가이드
- 거래량이 많은 종목, 거래대금이 많은 종목 Top 안내
- 더 많은 종목 볼수있는 화면으로 이동 배너 제공
...
```python
# -*- coding: utf-8 -*-
"""Hook script for skill `stock_volume_ranking` (자동 생성).
...
SKILL_NAME = 'stock_volume_ranking'
CASE_TYPE = 'normal'
FLOW = 'query'
REQUIRED_TOOLS = ['stock_ranking_inquiry']
