...
- `kelly_exchange_request(count: Optional[int] = None)` — 보유 켈리를 모니머니로 교환합니다 (실행형)
...
```python
# -*- coding: utf-8 -*-
"""Hook script for skill `kelly_exchange_request` (자동 생성).
...
SKILL_NAME = 'kelly_exchange_request'
CASE_TYPE = 'normal'
FLOW = 'action'
REQUIRED_TOOLS = ['kelly_balance_inquiry', 'kelly_exchange_request']
ACTION_TOOLS = ['kelly_exchange_request']    # 사용자 확인(confirmed=True) 없이는 호출 금지
