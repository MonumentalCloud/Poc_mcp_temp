---
name: jelly_investment_inquiry
description: '젤리투자 이용 중인 사용자가 지금까지의 누계 젤리 투자금액을 물을 때 사용합니다. 예: "오늘 젤리투자 신청했는데, 내가 지금까지 젤리투자로..."'
metadata:
  domain: monimo
  sector: non_finance
  case_type: normal
...
2. `jelly_investment_inquiry` 툴을 호출해 누계 젤리 투자금액과 투자 현황을 조회합니다.
...
## 응답 가이드
- 현재 일 기준, d-1일로 하여 '젤리보관함' 내 전환된 젤리 투자...
...
```python
# -*- coding: utf-8 -*-
"""Hook script for skill `jelly_investment_inquiry` (자동 생성).
...
SKILL_NAME = 'jelly_investment_inquiry'
CASE_TYPE = 'normal'
FLOW = 'query'
REQUIRED_TOOLS = ['jelly_investment_inquiry']
