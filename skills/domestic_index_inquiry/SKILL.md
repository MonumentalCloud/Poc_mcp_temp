...
국내/해외 주요지수와 환율의 등락을 조회해 안내한다.

## Instructions
1. 사용자 질의에서 조회 범위(국내/해외/전체/환율)를 파악합니다.
2. `market_index_inquiry` 툴을 호출해 국내/해외 주요지수와 환율의 현재가·등락을 조회합니다.
3. 지수별 현재가와 등락률, 대표 종목 흐름을 안내합니다. 환율은 최근 일주일 등락 추이를 함께 안내합니다.
4. 세부내용 확인 가능한 화면 이동 배너를 제공합니다.
...
- `market_index_inquiry(scope: str = 'all')` — 국내/해외 주요지수와 환율의 현재가·등락을 조회합니다

## 응답 가이드
- 국내 지수 당일 등락 종목수, 대표 종목 안내
- 세부 내용 확인 가능한 화면 이동배너 제공

## 예외 처리
- 조회 시점 기준 데이터임을 밝힙니다. 시장 전망·투자 판단은 제시하지 않습니다.
...
## 유저향 최종 안내 문구
조회 성공: "{지수명}은 {현재가}로 전일 대비 {등락률}% {상승/하락}했어요."
...
```python
# -*- coding: utf-8 -*-
"""Hook script for skill `domestic_index_inquiry` (자동 생성).
...
SKILL_NAME = 'domestic_index_inquiry'
CASE_TYPE = 'normal'
FLOW = 'query'
REQUIRED_TOOLS = ['market_index_inquiry']
