...
- `spending_category_inquiry(year_month: Optional[str] = None, period: Optional[str] = None, category: Optional[str] = None)` — 기준월/주차의 업종(카테고리)별 소비금액과 순위를 조회합니다
...
```python
# -*- coding: utf-8 -*-
"""Hook script for skill `monthly_top_category_inquiry` (자동 생성).
...
SKILL_NAME = 'monthly_top_category_inquiry'
CASE_TYPE = 'normal'
FLOW = 'query'
REQUIRED_TOOLS = ['spending_category_inquiry']
