...
2. `monischool_status_inquiry` 툴을 호출해 이번 달 모니스쿨 교시별 참여 이력(참여/정답 여부)을 조회합니다.
...
## 응답 가이드
- 이번 회원의 모니스쿨 최저별 참여 이력 제공 (참여 여부 및 정답 여부)
...
```python
# -*- coding: utf-8 -*-
"""Hook script for skill `monischool_status_inquiry` (자동 생성).
...
SKILL_NAME = 'monischool_status_inquiry'
CASE_TYPE = 'normal'
FLOW = 'query'
REQUIRED_TOOLS = ['monischool_status_inquiry']
