...
2. `temp_driver_inquiry` 툴을 호출해 임시운전자 특약 가입 내역을 조회합니다.
...
```python
# -*- coding: utf-8 -*-
"""Hook script for skill `temp_driver_status_inquiry` (자동 생성).
...
SKILL_NAME = 'temp_driver_status_inquiry'
CASE_TYPE = 'normal'
FLOW = 'query'
REQUIRED_TOOLS = ['temp_driver_inquiry']
ACTION_TOOLS = []    # 사용자 확인(confirmed=True) 없이는 호출 금지
PHRASES = ['조회 성공: "{항목} 정보를 안내드릴게요. {내용}"', '내역 없음: "조회된 {항목} 내역이 없어요."']
