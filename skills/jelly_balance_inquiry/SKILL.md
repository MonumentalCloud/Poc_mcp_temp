---
name: jelly_balance_inquiry
description: '사용자가 현재 보유한 젤리 개수(일반/스페셜)를 물을 때 사용합니다. 예: "나 지금 젤리 몇 개 있는지 알려줘"'
metadata:
  domain: monimo
  sector: non_finance
  case_type: normal
...
## 응답 가이드
- 현재 일자 기준 전환하지 않은 젤리 개수와 안내
- ex) 'mm.dd'일 기준 보유한 젤리는 총 0개예요. (일반젤리:nn개)
...
```python
# -*- coding: utf-8 -*-
"""Hook script for skill `jelly_balance_inquiry` (자동 생성).
...
SKILL_NAME = 'jelly_balance_inquiry'
CASE_TYPE = 'normal'
FLOW = 'query'
REQUIRED_TOOLS = ['jelly_balance_inquiry']
