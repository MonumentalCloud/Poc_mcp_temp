---
name: jelly_exchange_request
description: '사용자가 보유 젤리를 모니머니로 교환해 달라고 요청할 때 사용합니다. 보유 젤리 확인 후 교환 실행에 적합합니다. 예: "내 젤리 모니머니로 교환해줘"'
metadata:
  domain: monimo
  sector: non_finance
  case_type: normal
...
4. 사용자가 동의하면 `jelly_exchange_request` 툴을 호출해 보유 젤리를 모니머니로 교환합니다.
...
- `jelly_exchange_request(count: Optional[int] = None)` — 보유 젤리를 모니머니로 교환합니다 (실행형)

## 응답 가이드
- 현재 일자 기준 전환하지 않은 젤리 개수 안내 후 젤리 상점 화면으로 이동
