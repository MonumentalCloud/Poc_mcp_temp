...
- `temp_driver_apply(start_date: str, end_date: str)` — 임시운전자 특약을 지정 기간으로 가입합니다 (실행형)

## 응답 가이드
- 정확한 가입기간 확인
- 해당 가입일수 및 가입기간이 요청된 내용과 맞는지 확인
...
```python
# -*- coding: utf-8 -*-
"""Hook script for skill `temp_driver_apply_period` (자동 생성).
...
SKILL_NAME = 'temp_driver_apply_period'
CASE_TYPE = 'normal'
FLOW = 'action'
REQUIRED_TOOLS = ['temp_driver_apply']
ACTION_TOOLS = ['temp_driver_apply']    # 사용자 확인(confirmed=True) 없이는 호출 금지
