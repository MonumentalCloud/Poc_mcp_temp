---
name: kelly_challenge_apply_period_guide
description: '사용자가 참여 가능한 챌린지를 물었지만 현재 신청 가능 기간이 아닐 때 사용합니다. 신청 가능 기간(매월 16일~말일) 안내와 챌린지 메인 화면 이동에 적합합니다.
  예: "지금 참여가능한 챌린지를 알려줘"'
metadata:
  domain: monimo
  sector: non_finance
...
- `challenge_list_inquiry(status: str = 'available')` — 참여 중이거나 신청 가능한 켈리 챌린지 목록과 신청 가능 기간을 조회합니다

## 응답 가이드
- 챌린지 신청 가능 기간 안내 (매월 16일~말일 이내에만 차월 챌린지 신청 가능)- 켈린지 메인 화면으로 이동
...
```python
# -*- coding: utf-8 -*-
"""Hook script for skill `kelly_challenge_apply_period_guide` (자동 생성).
...
SKILL_NAME = 'kelly_challenge_apply_period_guide'
CASE_TYPE = 'normal'
FLOW = 'query'
REQUIRED_TOOLS = ['challenge_list_inquiry']
ACTION_TOOLS = []    # 사용자 확인(confirmed=True) 없이는 호출 금지
PHRASES = ['조회 성공: "{mm.dd}일 기준 보유한 켈리는 총 {n}개예요. (일반켈리: {n1}개, 스페셜켈리: {n2}개)"', '내역 없음: "{기준}에는 켈리 적립 내역이 없어요."']
