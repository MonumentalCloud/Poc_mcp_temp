...
4. 사용자가 동의하면 `bingo_start` 툴을 호출해 빙고게임 참여를 시작합니다 (실행형, 동의 필요).
...
```python
# -*- coding: utf-8 -*-
"""Hook script for skill `bingo_start_request` (자동 생성).
...
SKILL_NAME = 'bingo_start_request'
CASE_TYPE = 'normal'
FLOW = 'action'
REQUIRED_TOOLS = ['bingo_status_inquiry', 'bingo_start']
ACTION_TOOLS = ['bingo_start']    # 사용자 확인(confirmed=True) 없이는 호출 금지
