...
- `monthly_mission_participate(mission_name: str)` — 특정 이달의 미션 수행을 시작합니다 (실행형)
...
```python
# -*- coding: utf-8 -*-
"""Hook script for skill `monthly_mission_participate` (자동 생성).
...
SKILL_NAME = 'monthly_mission_participate'
CASE_TYPE = 'normal'
FLOW = 'action'
REQUIRED_TOOLS = ['monthly_mission_list_inquiry', 'monthly_mission_participate']
ACTION_TOOLS = ['monthly_mission_participate']    # 사용자 확인(confirmed=True) 없이는 호출 금지
