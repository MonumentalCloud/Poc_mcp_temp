...
4. 사용자가 동의하면 `interest_topic_update` 툴을 호출해 관심분야를 추가/삭제합니다.
...
- `interest_topic_update(action: str, topic: str)` — 관심분야를 추가/삭제합니다 (실행형)
...
```python
# -*- coding: utf-8 -*-
"""Hook script for skill `interest_topic_update` (자동 생성).
...
SKILL_NAME = 'interest_topic_update'
CASE_TYPE = 'normal'
FLOW = 'action'
REQUIRED_TOOLS = ['interest_topic_inquiry', 'interest_topic_update']
ACTION_TOOLS = ['interest_topic_update']    # 사용자 확인(confirmed=True) 없이는 호출 금지
