...
2. `product_detail` 툴을 호출해 상품 코드로 특정 상품의 상세 정보(보장내용/혜택/금리 등)를 조회합니다.
3. 조회 결과에서 사용자가 묻는 항목(보장내용/혜택/금리 등)을 추출해 요약합니다.
4. 상세내용 확인 가능한 화면 이동 배너를 함께 제공합니다.
...
## 응답 가이드
- 배달앱 10%할인 혜택 요약 안내(대상몰, 이용조건, 할인 제외)
...
```python
# -*- coding: utf-8 -*-
"""Hook script for skill `card_benefit_condition_inquiry` (자동 생성).
...
SKILL_NAME = 'card_benefit_condition_inquiry'
CASE_TYPE = 'normal'
FLOW = 'query'
REQUIRED_TOOLS = ['product_detail']
