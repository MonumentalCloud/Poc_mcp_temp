---
name: jelly_challenge_guide
description: '사용자가 젤리 챌린지가 무엇인지, 신청은 어떻게 하는지 등 서비스 전반을 물을 때 사용합니다. 예: "챌린지가 뭐야? 신청 어떻게 해?"'
metadata:
  domain: monimo
  sector: non_finance
  case_type: normal
...
3. `challenge_list_inquiry` 툴을 호출해 참여 중이거나 신청 가능한 젤리 챌린지 목록과 신청 가능 기간을 조회합니다.
...
- `faq_search(query: str, company: Optional[str] = None)` — 모니모/관계사 FAQ와 이용 가이드를 검색해 관련 문서를 조회합니다
- `challenge_list_inquiry(status: str = 'available')` — 참여 중이거나 신청 가능한 젤리 챌린지 목록과 신청 가능 기간을 조회합니다

## 응답 가이드
- 젤리 챌린지 서비스를 텍스트로 안내 (신청기간 포함)
...
```python
# -*- coding: utf-8 -*-
"""Hook script for skill `jelly_challenge_guide` (자동 생성).
...
SKILL_NAME = 'jelly_challenge_guide'
CASE_TYPE = 'normal'
FLOW = 'query'
REQUIRED_TOOLS = ['faq_search', 'challenge_list_inquiry']
ACTION_TOOLS = []    # 사용자 확인(confirmed=True) 없이는 호출 금지
PHRASES = ['안내 성공: "{질의 주제}에 대해 안내드릴게요. {요약 내용}"', '근거 없음: "정확한 안내를 위해 고객센터 확인이 필요한 내용이에요. 고객센터로 연결해 드릴까요?"']
