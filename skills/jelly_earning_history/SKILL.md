---
name: jelly_earning_history
description: '사용자가 특정 월의 젤리 적립내역(적립일자·이력·일반/스페셜 구분)을 물을 때 사용합니다. 예: "이번달 젤리 모은 내역 알려줘"'
metadata:
  domain: monimo
  sector: non_finance
  case_type: normal
...
2. `jelly_history_inquiry` 툴을 호출해 기준월의 젤리 적립/사용 내역을 조회합니다.
...
- `jelly_history_inquiry(year_month: Optional[str] = None)` — 기준월의 젤리 적립/사용 내역을 조회합니다

## 응답 가이드
- 현재 일 기준, '젤리보관함' 내 젤리 개수 합산하여 안내 ex) 'YY.MM...
...
```python
# -*- coding: utf-8 -*-
"""Hook script for skill `jelly_earning_history` (자동 생성).
...
SKILL_NAME = 'jelly_earning_history'
CASE_TYPE = 'normal'
FLOW = 'query'
REQUIRED_TOOLS = ['jelly_history_inquiry']
