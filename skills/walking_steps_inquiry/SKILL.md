...
2. `walking_steps_inquiry` 툴을 호출해 오늘/특정일 걸음수를 조회합니다.
...
- `walking_steps_inquiry(target_date: Optional[str] = None)` — 오늘/특정일 걸음수를 조회합니다

## 응답 가이드
- 매일 걸음수 조회 후 안내- 걷기 메인 화면으로 이동 후 안내
...
```python
# -*- coding: utf-8 -*-
"""Hook script for skill `walking_steps_inquiry` (자동 생성).
...
SKILL_NAME = 'walking_steps_inquiry'
CASE_TYPE = 'normal'
FLOW = 'query'
REQUIRED_TOOLS = ['walking_steps_inquiry']
