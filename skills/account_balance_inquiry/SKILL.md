...
3. `account_balance_inquiry` 툴을 호출해 마이통장 잔액을 조회합니다.
...
- `account_balance_inquiry(account_id: Optional[str] = None)` — 마이통장 잔액을 조회합니다

## 응답 가이드
- 보유 통장 여부 조회 필요요
- 보유 상품에 대해 먼저 조회 필요(KB국민은행 파킹 통장, 삼성증권 CMA)
...
```python
# -*- coding: utf-8 -*-
"""Hook script for skill `account_balance_inquiry` (자동 생성).
...
SKILL_NAME = 'account_balance_inquiry'
CASE_TYPE = 'normal'
FLOW = 'query'
REQUIRED_TOOLS = ['account_list_inquiry', 'account_balance_inquiry']
