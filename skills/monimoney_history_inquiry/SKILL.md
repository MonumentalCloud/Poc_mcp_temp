...
- `monimoney_history_inquiry(year_month: Optional[str] = None, tx_type: Optional[str] = None)` — 기준월의 모니머니 입출금/적립/차감 내역과 건별 사유를 조회합니다

## 응답 가이드
- 26년 4월 모니머니 적립 내역 합계 안내
- 모니머니 이용내역 목록으로 이동
...
```python
# -*- coding: utf-8 -*-
"""Hook script for skill `monimoney_history_inquiry` (자동 생성).
...
SKILL_NAME = 'monimoney_history_inquiry'
CASE_TYPE = 'normal'
FLOW = 'query'
REQUIRED_TOOLS = ['monimoney_history_inquiry']
ACTION_TOOLS = []    # 사용자 확인(confirmed=True) 없이는 호출 금지
PHRASES = ['조회 성공: "현재 보유하신 모니머니는 {금액}원이에요."', '내역 안내: "{기준} 모니머니 {유형} 내역은 총 {금액}원({건수}건)이에요."']
