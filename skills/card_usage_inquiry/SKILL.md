...
- `card_usage_inquiry(card_name: Optional[str] = None, period: Optional[str] = None)` — 특정 카드의 이용내역(기본 최근 2주)을 조회합니다
...
```python
# -*- coding: utf-8 -*-
"""Hook script for skill `card_usage_inquiry` (자동 생성).
...
SKILL_NAME = 'card_usage_inquiry'
CASE_TYPE = 'normal'
FLOW = 'query'
REQUIRED_TOOLS = ['card_list_inquiry', 'card_usage_inquiry']
ACTION_TOOLS = []    # 사용자 확인(confirmed=True) 없이는 호출 금지
PHRASES = ['조회 성공: "{카드명}의 최근 2주 이용내역은 총 {건수}건, {금액}원이에요."', '결제금액: "이번 결제일({일자})에 출금 예정인 금액은 {금액}원이에요."']
