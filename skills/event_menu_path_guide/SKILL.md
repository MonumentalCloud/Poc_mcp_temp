...
4. 이벤트명·기간·혜택을 요약해 리스트로 안내하고, 이벤트 화면 이동 배너를 제공합니다.
...
## 응답 가이드
- 생명: '연금' 키워드가 포함된 이벤트 리스트 제공(요약정보 표기)
...
```python
# -*- coding: utf-8 -*-
"""Hook script for skill `event_menu_path_guide` (자동 생성).
...
SKILL_NAME = 'event_menu_path_guide'
CASE_TYPE = 'normal'
FLOW = 'query'
REQUIRED_TOOLS = ['event_list_inquiry', 'menu_search']
ACTION_TOOLS = []    # 사용자 확인(confirmed=True) 없이는 호출 금지
PHRASES = ['조회 성공: "지금 진행 중인 이벤트는 {n}개예요. 1. {이벤트명} ({기간}) — {혜택 요약} ..."', '결과 없음: "현재 조건에 맞는 진행 중인 이벤트가 없어요."']
