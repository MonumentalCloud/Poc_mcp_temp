...
- `receipt_issue_request(contract_id: str, receipt_type: str = '해지')` — 해지/만기 영수증 발급을 신청합니다 (실행형, 자동차보험 불가)
...
```python
# -*- coding: utf-8 -*-
"""Hook script for skill `cancellation_receipt_issue` (자동 생성).
...
SKILL_NAME = 'cancellation_receipt_issue'
CASE_TYPE = 'normal'
FLOW = 'action'
REQUIRED_TOOLS = ['insurance_contract_inquiry', 'receipt_issue_request']
ACTION_TOOLS = ['receipt_issue_request']    # 사용자 확인(confirmed=True) 없이는 호출 금지
