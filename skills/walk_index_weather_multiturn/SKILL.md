...
```python
# -*- coding: utf-8 -*-
"""Hook script for skill `walk_index_weather_multiturn` (자동 생성).
...
SKILL_NAME = 'walk_index_weather_multiturn'
CASE_TYPE = 'multiturn'
FLOW = 'query'
REQUIRED_TOOLS = ['weather_inquiry']
ACTION_TOOLS = []    # 사용자 확인(confirmed=True) 없이는 호출 금지
PHRASES = ['지역 확인: "어느 지역의 날씨를 알려드릴까요?"', '안내: "{지역}의 날씨는 {날씨}입니다. 최고 기온은 {max}도, 최저 기온은 {min}도입니다."']
