...
3. `spending_summary_inquiry` 툴을 호출해 기준월/주차의 총 소비금액과 결제수단별·일별 내역, 전월 대비 증감을 조회합니다.
4. 조회 결과에서 질의에 해당하는 값(총액/최다 카테고리/카드별 금액 등)을 추출해 안내하고, 비교 정보(전월 대비 등)가 있으면 함께 제공합니다.
...
```python
# -*- coding: utf-8 -*-
"""Hook script for skill `budget_status_inquiry` (자동 생성).
...
SKILL_NAME = 'budget_status_inquiry'
CASE_TYPE = 'normal'
FLOW = 'query'
REQUIRED_TOOLS = ['budget_inquiry', 'spending_summary_inquiry']
