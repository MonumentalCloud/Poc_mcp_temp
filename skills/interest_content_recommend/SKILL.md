...
3. `content_list_inquiry` 툴을 호출해 인기소식 또는 관심분야별 컨텐츠 목록을 조회합니다.
...
## 응답 가이드
- 고객이 설정한 관심소식의 목록을 알려줌 ex) 이슈산악·캘기맺각, 소비자율 푸른소리
...
```python
# -*- coding: utf-8 -*-
"""Hook script for skill `interest_content_recommend` (자동 생성).
...
SKILL_NAME = 'interest_content_recommend'
CASE_TYPE = 'normal'
FLOW = 'query'
REQUIRED_TOOLS = ['interest_topic_inquiry', 'content_list_inquiry']
