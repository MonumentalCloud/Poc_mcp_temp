...
3. `kelly_level_inquiry` 툴을 호출해 켈리 적립 레벨·회원등급과 모니머니 전환 비율을 조회합니다.
4. 조회 결과를 일반켈리/스페셜켈리 구분 등 세부 기준과 함께 안내하고, 켈리 상점·챌린지 화면 이동 배너를 제공합니다.
...
```python
# -*- coding: utf-8 -*-
"""Hook script for skill `kelly_conversion_rate_inquiry` (자동 생성).
...
SKILL_NAME = 'kelly_conversion_rate_inquiry'
CASE_TYPE = 'normal'
FLOW = 'query'
REQUIRED_TOOLS = ['kelly_balance_inquiry', 'kelly_level_inquiry']
