...
3. `content_detail_inquiry` 툴을 호출해 특정 컨텐츠의 본문 내용을 조회합니다.
4. 조회된 컨텐츠의 제목·내용을 요약 안내하고, 해당 컨텐츠/설정 화면 이동 배너를 제공합니다.
...
- `content_list_inquiry(sort: str = 'popular', topic: Optional[str] = None)` — 인기소식 또는 관심분야별 컨텐츠 목록을 조회합니다
- `content_detail_inquiry(content_id: str)` — 특정 컨텐츠의 본문 내용을 조회합니다
...
```python
# -*- coding: utf-8 -*-
"""Hook script for skill `content_detail_inquiry` (자동 생성).
...
SKILL_NAME = 'content_detail_inquiry'
CASE_TYPE = 'normal'
FLOW = 'query'
REQUIRED_TOOLS = ['content_list_inquiry', 'content_detail_inquiry']
ACTION_TOOLS = []    # 사용자 확인(confirmed=True) 없이는 호출 금지
PHRASES = ['조회 성공: "오늘의 {기준} 소식이에요. 1. {제목1} — {요약1} ..."', '결과 없음: "지금 보여드릴 {기준} 컨텐츠가 없어요."']
